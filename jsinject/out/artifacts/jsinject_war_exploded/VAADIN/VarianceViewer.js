...
var VarianceViewer = function (theBody, options) {
	//	INIT
	var main = this;
	main.cfg = $.extend({},  $.fn.VarianceViewer.defaults, options);
	...
	main.galleryNode.addClass('ae-va-loading');
	main.galleryNode.addClass("ae-va-viewer-block");
	
	main.items = [];
	main.containerPosition = 0;
	main.moverMargin = 0;
	main.galleryNode.addClass('ae-va-viewer');
	main.mover = $("<div class='ae-va-mover'></div>");
	main.galleryWidth = 0;
	...
	main.lightbox = new function(){
		...
		lObject.preset = function(){
			...
			lObject.isPreset = 1;
			lObject.node = $("<div class='ae-va-lightbox'></div>");
			lObject.bg = $("<div class='ae-va-lightbox-bg'></div>");
		...
		lObject.show = function(){
			if(lObject.onScreen){return;}
			if(AE.currentBALightbox){return;}
			AE.currentBALightbox = this;
			lObject.preset();
			main.placeHolder = main.placeHolder || $("<div class='ae-va-placeholder' style='display:none;'></div>");
			...
			lObject.node.slideDown({
				...
				complete:function(){
					...
					main.galleryNode.addClass("ae-va-zoom-mode");
					main.galleryNode.removeClass("ae-va-viewer-block");
					main.zoomMode = 1;
		...
		lObject.hide = function(){
			...
			main.galleryNode.fadeOut(500, function(){
				AE.currentBALightbox = 0;
				main.zoomMode = 0;
				lObject.onScreen = 0;				
				main.galleryNode.detach();
				main.galleryNode.removeClass("ae-va-zoom-mode");
				main.galleryNode.addClass("ae-va-viewer-block");
				if(!main.onlyLightbox){
	...
	main.updateTitle = function(str, url, urlTarget){
		...
		main.textNode = main.textNode || $('.ae-va-info-text', main.galleryNode);
		if(!str.length){
			main.info.addClass('ae-va-info-hide')
		} else {
			main.info.removeClass('ae-va-info-hide');
			
			if(url){
				if(!urlTarget){
					urlTarget = "_self";
				}
				str = "<a class='ae-va-url' href='"+url+"' target='"+urlTarget+"'> &raquo; " + str + "</a>";
			}
	...
	//BA VIEWER
	var vaViewer = function(baNode, baIndex){
		var viewer = this;
		viewer.picA = {};
		viewer.picB = {};
		viewer.loaded = 0;
		...
		viewer.introAnim = function(){
			...
			if(main.cfg.disableIntro === true){
				$(viewer).delay(200 * AE.vaViewersCount).animate({
					...
					complete:function(){
							viewer.update();
							AE.vaViewersCount -=1;
							if(AE.vaViewersCount < 0){AE.vaViewersCount = 0;}
						}
				});
			} else {
				$(viewer).delay(300 + 300 * AE.vaViewersCount).animate({
					percent: 0
				}, {
					duration: main.cfg.animSpeed,
					step:function(){
		...
		viewer.showA = function(){
			if(viewer.picA.isLoaded() && !viewer.picA.onScreen){
				...
				main.galleryNode.removeClass('ae-va-loading');
				viewer.node.append(viewer.picA.pic);
				viewer.picA.pic.fadeOut(0);
				viewer.picA.pic.fadeIn(300);
				viewer.picA.pic.addClass("ae-va-pic-a");
				viewer.picA.pic.addClass("ae-va-pic");
				if(viewer.onScreen){
		...
		viewer.showB = function(){
			if(viewer.picB.isLoaded() && !viewer.picB.onScreen){
				...
				viewer.door = $("<div class='ae-va-door'></div>");
				viewer.mask = $("<div class='ae-va-mask'></div>");
				viewer.picBox = $("<div class='ae-va-pic-box'></div>");
				viewer.wall = $("<div class='ae-va-wall'></div>");
				viewer.constroller = $('<div class="ae-va-controller"><div class="ae-va-controller-sign"></div></div>');
				...
				viewer.picB.pic.addClass("ae-va-pic-b");
				viewer.picB.pic.addClass("ae-va-pic");

				viewer.node.append(viewer.wall);
				dragStuff = new AE.DragStuff({
		...
		viewer.preset = function(){
			...
			viewer.isPreset = 1;
			viewer.node = $('<div class="ae-va-box"></div>');
			viewer.preloader = $('<div class="ae-va-preloader"></div>');
		...
		var imageObject = function(picPath){
			...
			picObject.resize = function(){
				...
				if(main.lightbox.onScreen){
					if(picObject.pic.width() > AE.cW - screenMargin){
						picObject.pic.css("width", AE.cW - screenMargin);
						picObject.pic.css("height", "auto");
					}
					
					if(picObject.pic.height() > AE.cH - screenMargin){
						picObject.pic.css("height", AE.cH - screenMargin);
						picObject.pic.css("width", "auto");
					}
					
					picObject.pic.css("left", parseInt(AE.cW/2 - picObject.pic.width()/2, 10));
					picObject.pic.css("top", parseInt(AE.cH/2 - picObject.pic.height()/2, 10));
	...
	main.imageNodes.each(function(baIndex){
		var ba = new vaViewer($(this), baIndex);
	});
	...
	main.setSize = function(){
		...
		if(main.lightbox.onScreen){
			...
			main.galleryWidth = AE.cW;
			main.galleryHeight = AE.cH;
	...
	main.preset = function(){
		...
		//
		if(main.nrItems > 1){
			main.footer = $("<div class='ae-va-footer'></div>");
			if(!main.cfg.hideTitles){
				main.info = $("<div class='ae-va-info'><div class='ae-va-info-bg'></div><div class='ae-va-info-text'>DEMO Title!</div></div>");
			}
			...
			main.btnNext = $("<div class='ae-va-next'></div>");
			main.btnPrev = $("<div class='ae-va-prev'></div>");
	...
	if(main.galleryNode.attr('data-lightbox')){
		main.btnZoom = $('<div class="ae-va-btn-zoom"></div>');
		...
		if(main.galleryNode.attr('data-lightbox') === "default+lightbox" || main.galleryNode.attr('data-lightbox') === "simple+lightbox"){
			...
			if(!lb.length){
				main.btnZoom = $('<div class="ae-va-btn-open"></div>');
...
$.fn.VarianceViewer = function(options) {
    return this.each(function(){
        var element = $(this);
        // Return early if this element already has a plugin instance
        if (element.data('varianceviewer')) {
        	return;
        }
        // Pass options to plugin constructor
        var varianceviewer = new VarianceViewer(this, options);
        // Store plugin object in this element's data
        element.data('varianceviewer', varianceviewer);
    });
};

//Default settings
$.fn.VarianceViewer.defaults = {
	width:"auto",
	height:"auto",
	animSpeed: 900,
	maxWidth: 0,
...
AE.localResizeEngine = function(){
	var main = this;
	var $ = jQuery;
	main.resizeTimer = 0;
	
	if(AE.ResizeEngineStarter){ return; };
	AE.ResizeEngineStarter = true;
	main.resizeEvent = function(){
		clearInterval(main.resizeTimer);
		main.resizeTimer = setInterval(function(){
			if(!AE.body){
				if($('body').length){AE.body = $('body');}
			}
			
			AE.cW = $(window).width();
			AE.cH = $(window).height();
			
			AE.body.trigger('WindowResized');
			clearInterval(main.resizeTimer);
		}, 100);
	}
...
AE.ResizeEngine = AE.ResizeEngine || AE.localResizeEngine;
...
var AEmodule = {
	...
	func : function(cfg){
		...
		var touchEventsPreset = function(){
			...
			AE.body.bind("touchmove", function(e){
			...
			AE.body.bind("touchend", function(e){ touchEnd(e); });
			AE.body.bind("touchcancel", function(e){ touchEnd(e); });
		...
		main.mouseUp = function(e){
			...
			AE.body.unbind('mouseup', main.mouseUp);
			AE.body.unbind('mousemove', main.mouseMove);
		...
		main.mouseDown = function(e){
			...
			AE.body.mousemove(main.mouseMove);
		...
		var dragStart = function(){
			...
			cfg.theTrigger.trigger('aeDragStart');
		}
		...
		var dragEnd = function(e){
			...
			dragDistance = 0;
			dragDistanceV = 0;
			cfg.theTrigger.trigger('aeDragEnd');
		...
		main.inertiaStep = function(){
			main.inertiaX = AE.smoothMove(main.inertiaX, 0, 15);
...
if(!AE[AEmodule.id+"V"] || AE[AEmodule.id+"V"] < AEmodule.version) {
	AE[AEmodule.id] = AEmodule.func;
	AE[AEmodule.id+"V"] = AEmodule.version;
} 

AE.smoothMove = function(curr, tar, spd){
